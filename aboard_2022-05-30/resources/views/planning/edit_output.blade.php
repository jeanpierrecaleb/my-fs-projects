...
@section('content')
    <div class="page-wrapper">
        <div class="content container-fluid">
                ...
                <div class="row">
                    <div class="col-sm-12 mt-5">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"> Planning > Edit Output </li>
            ...
            <form action="{{ url('planning/output/update/' . $output->id) }}" method="POST" id="myform"  enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="mb-3">
                    <label for="name" class="form-label">Output Name</label>
                    <input type="text" id="name" class="form-control" name="name" value="{{ $output->name }}"
                        placeholder="Write the name of the directorate" required>
                </div>

                <div class="mb-3">
                    <label class="form-label">Belonging Sub-Program</label>
                    <select id="subprogram_id" name="subprogram_id" class="form-select" required form="myform">
                        <option value=" " disabled>Select below</option>

                        <option value="{{$output->subprogram_id}}"> {{ $output->subprogram_id }},
                            {{ $output->subprogram->name }} </option>

                    </select>
                </div>
                {{-- Id du Work Plan en hidden --}}
                <div class="mb-3">
                    <input type="hidden" id="prog_id" class="form-control" name="prog_id" placeholder="Program id"
                        value="{{ $output->subprogram->program_id }}">
                </div>

                <div class="mb-3">
                    <label class="form-label">Status</label>
                    <select id="status" name="status" form="myform" class="form-select" aria-label="Status applied by default">
                        <option value="{{ $output->status }}" selected> {{ $output->status }} </option>
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label">Quater</label>
                    <select id="quater" name="quater" class="form-select" form="myform" aria-label="Quater applied">
                        <option value="{{ $output->quater }}" selected> {{ $output->quater }} </option>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" form="myform" class="form-control" id="description" rows="3"
                        placeholder="Add any extra details about the request"> {{ $output->description }} </textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label">Linked Outcome</label>
                    <select id="outcome_id" name="outcome_id" form="myform" class="form-select"
                        aria-label="Select an Outcome">
                        <option value=" " disabled>Select below</option>
                        <option value="{{ $output->outcome_id }}" selected>{{ $output->outcome_id }}
                            @foreach ($output->subprogram->program->outcomes as $outcome)
                        <option value="{{ $outcome->id }}">{{ $outcome->id }}, {{ $outcome->name }}
                            @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="risks" form="myform" class="form-control" id="risks" rows="3"
                        placeholder="Add any risks"> {{ $output->risks }} </textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" value="{{ $output->indicators }}" id="indicators" class="form-control"
                ...
                <div class="modal-body">
                    <form action="{{ url('planning/create-ocindicator') }}" method="POST" id="myform2">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" value="{{ $outcome->name }}" id="name" name="name"
