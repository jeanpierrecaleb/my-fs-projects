...
@section('content')
    <div class="page-wrapper">
                ...
                <div class="row">
                    <div class="col-sm-12 mt-5">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"> Community > Edit and update WPA </li>
        ...
        <form action="{{ url('planning/wpa/update/'.$wpa->id) }}" method="POST" id="myform"  enctype="multipart/form-data"  >
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="mb-3">
                <label for="name" class="form-label">Work Plan Name</label>
            ...
            <div class="mb-3">
                ...
                <select id="directorate_id" name="directorate_id" class="form-select" aria-label="Select a Directorate">
                    ...
                    @foreach ($directorates as $item)
                        <option value="{{ $item->id}}">{{  $item->id }}, {{$item->name }}, {{$item->department->name }}, {{ $item->city }}</option>
                    @endforeach
                    @else
                    <option value="{{ $directorate->id}}">{{  $directorate->id }}, {{$directorate->name }}, {{ $directorate->city }}, {{$directorate->department->name }} </option>
                    @endif
